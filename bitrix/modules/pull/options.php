...
<?endif;?>
	...
	<tr>
		...
		<td><input id="config_path_to_listener_secure" type="text" size="40" value="<?=htmlspecialcharsbx(CPullOptions::GetListenSecureUrl())?>" name="path_to_listener_secure" <?=(CPullOptions::GetNginxStatus()? '':'disabled="true"')?>></td>
	</tr>
	<?/*
	<tr>
	...
	<tr>
		...
		<td><input id="config_path_to_websocket_secure" type="text" size="40" value="<?=htmlspecialcharsbx(CPullOptions::GetWebSocketSecureUrl())?>" name="path_to_websocket_secure" <?=(!CPullOptions::GetNginxStatus() || !CPullOptions::GetWebSocketStatus() ? 'disabled="true"': '')?></td>
	</tr>
	*/?>
	<tr>
...
BX.bind(BX('config_websocket'), 'change', function(){
	if (this.checked)
	{
		if (confirm("<?=GetMessageJS("PULL_OPTIONS_WS_CONFIRM")?>"))
		{
			BX('config_path_to_websocket').disabled = false;
			BX('config_path_to_websocket_secure').disabled = false;
		}
