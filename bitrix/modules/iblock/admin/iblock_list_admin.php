		...
		if($TYPE=="E")
		{
			...
			if($bCatalog)
			{
				if ($boolCatalogPrice && CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $ID, "element_edit_price"))
				{
					...
					if (array_key_exists('CATALOG_WEIGHT', $arFields) && '' != $arFields['CATALOG_WEIGHT'])
						$arCatalogProduct['WEIGHT'] = $arFields['CATALOG_WEIGHT'];
					if (array_key_exists('CATALOG_VAT_INCLUDED', $arFields) && !empty($arFields['CATALOG_VAT_INCLUDED']))
						$arCatalogProduct['VAT_INCLUDED'] = $arFields['CATALOG_VAT_INCLUDED'];
					if (array_key_exists('CATALOG_QUANTITY_TRACE', $arFields) && !empty($arFields['CATALOG_QUANTITY_TRACE']))
						$arCatalogProduct['QUANTITY_TRACE'] = $arFields['CATALOG_QUANTITY_TRACE'];
					if ('Y' != $strUseStoreControl)
					{
						if (array_key_exists('CATALOG_QUANTITY', $arFields) && '' != $arFields['CATALOG_QUANTITY'])
...
if ($strUseStoreControl == "Y" && in_array("CATALOG_BAR_CODE", $arSelectedFields) && !empty($arElemID))
{
	...
	if (!empty($productsWithBarCode))
	{
		...
		while ($product = $rsProducts->Fetch())
		{
			if (isset($arRows['E'.$product["PRODUCT_ID"]]))
			{
				$arRows['E'.$product["PRODUCT_ID"]]->arRes["CATALOG_BAR_CODE"] = htmlspecialcharsEx($product["BARCODE"]);
			}
		}
	}
}
