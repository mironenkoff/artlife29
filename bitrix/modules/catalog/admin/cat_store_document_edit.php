...
if($_SERVER['REQUEST_METHOD'] == "POST" && check_bitrix_sessid() && isset($_POST["DOCUMENT_TYPE"]))
{
	...
	if(isset($_POST["PRODUCT"]) && is_array($_POST["PRODUCT"]) && $docId)
	{
		...
		{
			LocalRedirect("/bitrix/admin/cat_store_document_edit.php?lang=".LANGUAGE_ID."&ID=".$docId."&action=cancellation&sessid=".$_REQUEST["sessid"]."&".GetFilterParams("filter_", false));
		}
		...
		$arProducts = array_reverse($_POST["PRODUCT"]);
		...
		{
			LocalRedirect("/bitrix/admin/cat_store_document_edit.php?lang=".LANGUAGE_ID."&ID=".$docId."&action=conduct&sessid=".$_REQUEST["sessid"]."&".GetFilterParams("filter_", false));
		}
...
$tabControl->Begin();
?>
	<form method="POST" action="/bitrix/admin/cat_store_document_edit.php" enctype="multipart/form-data" id="form_catalog_document_form" name="form_catalog_document_form"><div class="adm-detail-block" id="form_order_buyers_layout" style="min-width: 165px;">
		...
		<td colspan="2">
			<table cellpadding="3" cellspacing="1" border="0" width="100%" class="internal" id="CAT_DOC_TABLE">
				<tr class="heading">
					...
					<?endif;?>
					<?if(is_set($requiredFields, "BAR_CODE")):?>
			...
			<br>
			<div style="text-align: right;  font-weight:bold; font-size:14px; font-family:Tahoma,Verdana,Arial,sans-serif; word-spacing: 5px;">
			<?if($docType === DOC_ARRIVAL):?>
				<?=GetMessage("CAT_DOC_TOTAL")?>:<span id="CAT_DOC_TOTAL">&#9;0</span><span style="font-size: small; padding-right: 30%;" id="CAT_DOC_TOTAL_CURRENCY"></span>
			<?endif;?>
			<?if(!$bReadOnly):?>
				<span style="right:5%;"> <a title="<?=GetMessage("CAT_DOC_ADD_ITEMS")?>" onClick="AddProductSearch(1);"  class="adm-btn adm-btn-green adm-btn-add" href="javascript:void(0);"><?=GetMessage("CAT_DOC_ADD_ITEMS")?></a></span>
			<?endif;?>
			</div>
		</td>
	</tr>
		...
		<td valign="top" align="left" colspan="2">
			...
			<div style="float:right">
				<script>
					...
					{
						dateURL = '<?=bitrix_sessid_get()?>&ORDER_AJAX=Y&BARCODE='+barcode+'&lang=<? echo LANGUAGE_ID; ?>';
...
//    oCellBalance.setAttribute('class','balance_count');
		...
		<?if(!$bReadOnly):?>
		oCellQuantity.innerHTML = "<div align='center'><input maxlength=\"10\" onChange=\"fRecalProduct(" + ID + ", '', 'N', 'N');\" type=\"text\" name=\"PRODUCT[" + ID + "][QUANTITY]\" id=\"PRODUCT[" + ID + "][QUANTITY]\" value=\"" + quantity + "\" size=\"4\"></div>";
		<?else:?>
		...
		<?if(is_set($requiredFields, "NET_PRICE")):
			$CURRENCY_FORMAT = '';
			if (array_key_exists('CURRENCY_FORMAT', $_REQUEST))
			{
				$CURRENCY_FORMAT = strval($_REQUEST['CURRENCY_FORMAT']);
			}
		?>
		var priceColumn = "";
		if (!valutaFormat) valutaFormat = '<? echo CUtil::JSEscape($CURRENCY_FORMAT); ?>';
		...
		<?if(!$bReadOnly):?>
		priceColumn += "<input maxlength=\"15\" style='text-align: center;' align='center' onblur=\"fEditPrice('" + ID + "', 'exit');\" onclick=\"fEditPrice('" + ID + "', 'on');\" onchange=\"fRecalProduct('" + ID + "', 'price', 'N', 'N');\" type=\"text\" name=\"PRODUCT[" + ID + "][PRICE]\" id=\"PRODUCT[" + ID + "][PRICE]\" value=\"" + number_format(price) + "\" size=\"5\" >";
		<?else:?>
		priceColumn += "<span id=\"PRODUCT[" + ID + "][PRICE_SPAN]\">"+number_format(price)+"</span>";
		<?endif;?>
		priceColumn += "</span>";
		priceColumn += "<span id='currency_price_product' class='currency_price'>"+BX.util.htmlspecialchars(valutaFormat)+"</span>";
		priceColumn += "</div>";
		priceColumn += "<div align='center' id=\"DIV_PRICE_OLD_"+ID+"\" class=\"base_price\" style=\"display:none;\">" + priceBaseFormat + " <span>"+BX.util.htmlspecialchars(valutaFormat)+"</span></div>";

		priceColumn += "<div align='center' id=\"DIV_BASE_PRICE_WITH_DISCOUNT_"+ID+"\" class=\"base_price\">";

		if (discountPercent > 0)
			priceColumn += priceBaseFormat + "<span>"+BX.util.htmlspecialchars(valutaFormat)+"</span>";
		...
		if(isMultiBarcode == 'Y' && oCellBarcode)
		{
			var oCell, oCellDel;
	...
	function fRecalProduct(id, type, recommendet, recalcAll)
	{
		...
		{
			product_price = BX("PRODUCT["+id+"][PRICE]").value.replace(/ /g, '');
		...
		{
			product_price = BX("PRODUCT["+id+"][PRICE_SPAN]").innerHTML.replace(/ /g, '');
			if(!isNaN(product_price * product_quantity))
				showPrice = product_price * product_quantity;
			BX('DIV_SUMMA_' + id).innerHTML = "<div align='center'><span id='CAT_DOC_SUMM_"+id+"'>" + number_format(showPrice) + "</span></div>";
		}
...
<?if($ID > 0)
{
	...
	while($product = $dbResult->GetNext())
	{
		...
		$arProductInfo = CCatalogStoreControlUtil::getProductInfo($product["ELEMENT_ID"]);
		if(is_array($arProductInfo))
			$product = array_merge($product, $arProductInfo);
		$product["BARCODE"] = '';
