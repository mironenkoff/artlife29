...
<tr class="heading">
	...
	?><script type="text/javascript">
			...
			{
				var obTrBarCode = BX('tr_CAT_BARCODE');
				var obTrBarCodeEdit = BX('tr_CAT_BARCODE_EDIT');
				if(BX('CAT_BARCODE_MULTIPLY').checked)
				{
					if (!!obTrBarCode)
					{
						obTrBarCode.disabled = true;
						obTrBarCode.style.display = 'none';
					}
					if (!!obTrBarCodeEdit)
					{
						obTrBarCodeEdit.disabled = true;
						obTrBarCodeEdit.style.display = 'none';
					}
				}
				else
				{
					if (!!obTrBarCode)
					{
						obTrBarCode.disabled = false;
						obTrBarCode.style.display = 'table-row';
					}
					if (!!obTrBarCodeEdit)
					{
						obTrBarCodeEdit.disabled = false;
						obTrBarCodeEdit.style.display = 'table-row';
					}
				}
			}
	...
	if($bUseStoreControl)
	{
		...
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="edit-table" id="catalog_barcode_table">
			...
			<tr id="tr_CAT_BARCODE"<? echo (($bUseMultiplyBarcode) ? ' style="display: none;"' : ''); ?>>
				...
				?>
				<td><input type="text" name="CAT_BARCODE" id="CAT_BARCODE" size="30" value="<?=htmlspecialcharsbx($barcode); ?>" <? echo $strDisable; ?>/></td>
			</tr><?
			if (0 < $PRODUCT_ID && '' != $barcode)
			{
			?>
			<tr id="tr_CAT_BARCODE_EDIT"<? echo (($bUseMultiplyBarcode) ? ' style="display: none;"' : ''); ?>>
				<td><?echo GetMessage("C2IT_BAR_CODE_EDIT")?>:</td>
				<td>
					<input type="hidden" name="CAT_BARCODE_EDIT" id="CAT_BARCODE_EDIT_N" value="N" />
