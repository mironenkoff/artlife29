		...
		if (CIBlockElementRights::UserHasRightTo($IBLOCK_ID, $PRODUCT_ID, "element_edit_price"))
		{
			...
			if (strlen($strWarning) <= 0)
			{
				...
				elseif(isset($_REQUEST["CAT_BARCODE"]) && $barcodeMultiply != 'Y')
				{
					...
					if(count($arId) == 1)
					{
						if (isset($_REQUEST['CAT_BARCODE_EDIT']) && 'Y' == $_REQUEST['CAT_BARCODE_EDIT'])
						{
							if ('' != trim($_REQUEST["CAT_BARCODE"]))
							{
								if (!CCatalogStoreBarCode::Update($arId[0], array("MODIFIED_BY" => $userId, "STORE_ID" => 0, "BARCODE" => trim($_REQUEST["CAT_BARCODE"]))))
