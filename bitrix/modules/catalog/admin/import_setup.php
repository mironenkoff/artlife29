...
if (($bCanEdit || $bCanExec) && check_bitrix_sessid())
{
	...
	if (strlen($strErrorMessage)<=0 && strlen($strActFileName)>0)
	{
		...
		if (strlen($strErrorMessage)<=0)
		{
			...
			{
				$CUR_LOAD_SESS_ID = '';
				if (array_key_exists('CUR_LOAD_SESS_ID', $_REQUEST))
				{
				...
				$CUR_FILE_POS = 0;
				if (array_key_exists('CUR_FILE_POS', $_REQUEST))
				{
				...
				if ($CUR_FILE_POS > 0 && '' != $CUR_LOAD_SESS_ID && array_key_exists($CUR_LOAD_SESS_ID, $_SESSION) && is_array($_SESSION[$CUR_LOAD_SESS_ID]))
				{
					$bFirstLoadStep = false;

					$arSetupVars = array();
					$intSetupVarsCount = 0;
					if (array_key_exists("SETUP_VARS", $_SESSION[$CUR_LOAD_SESS_ID]))
					{
					...
					if (array_key_exists("INTERNAL_VARS", $_SESSION[$CUR_LOAD_SESS_ID]))
					{
				...
				else
				{
					...
					if ('' != $CUR_LOAD_SESS_ID && array_key_exists($CUR_LOAD_SESS_ID, $_SESSION))
					{
