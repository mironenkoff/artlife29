		...
		if(!empty($arProductId))
		{
			...
			while($arProduct = $dbProduct->GetNext())
			{
				...
				$result .= '<tr><td class="tab_img">'.$imgProduct.'</td><td class="tab_text">
									...
									<div class="order_price">'.GetMessage('SOD_ORDER_RECOM_PRICE').': <b>'.SaleFormatCurrency($arProduct["PRICE"], $currency).'</b></div>';
					...
					<td colspan="2" valign="top">
						...
						if(!empty($arBasketId))
						{
							...
							<tr>
								<td class="photo">
									...
									if ($bModuleInclud)
									{
										...
										if ($arProductInfo["IBLOCK_ID"] > 0)
											$arBasket["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$arBasket["PRODUCT_ID"]."&type=".$arProductInfo["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arProductInfo["IBLOCK_ID"]."&find_section_section=".IntVal($arProductInfo["IBLOCK_SECTION_ID"]);
									...
									if (is_array($arBasket["PICTURE"]))
											echo '<img src="'.$arBasket["PICTURE"]["src"].'" alt="" border="0" />';
										else
								...
								<td class="props">
									<?
									if (!empty($arBasketProps[$arBasket["ID"]]) && is_array($arBasketProps[$arBasket["ID"]]))
									{
								...
								<td class="order_price" nowrap>
										...
										if (0 < $priceDiscount)
										{
											...
											if ('Y' != $arBasket["CUSTOM_PRICE"])
											{
												?><div class="discount">(<? echo GetMessage('SOD_PRICE_DISCOUNT')." ".$priceDiscount?>%)</div><?
											}
										}
										?><div class="base_price_title">
											<?=('Y' == $arBasket["CUSTOM_PRICE"]) ? GetMessage("SOD_BASE_CATALOG_PRICE") : $arBasket["NOTES"];?>
										</div>
								</td>
