...
if (
	...
	&& check_bitrix_sessid()
	&& (
		(array_key_exists('save', $_POST) && '' != $_POST['save'])
		|| (array_key_exists('apply', $_POST) && '' != $_POST['apply'])
	)
)
{
	$NEW_ID = '';
	if (array_key_exists('NEW_ID', $_POST))
	...
	foreach ($arLangIDs as &$strLangID)
	{
		...
		if (array_key_exists('NAME_'.$strLangID, $_REQUEST))
			$arLangName[$strLangID]['NAME'] = trim($_REQUEST['NAME_'.$strLangID]);
		if (array_key_exists('DESCRIPTION_'.$strLangID, $_REQUEST))
	...
	if (!empty($arStatusGroupIDs))
	{
		...
		foreach ($arStatusGroupIDs as &$intOneGroupID)
		{
			...
			foreach ($arPermFieldKeys as &$strKey)
			{
				if ('GROUP_ID' == $strKey)
					continue;
				if (array_key_exists($strKey.'_'.$intOneGroupID, $_REQUEST) && 'Y' == $_REQUEST[$strKey.'_'.$intOneGroupID])
				{
					$boolViewPerm = true;
	...
	$arFields = array(
		'ID' => ('' != $ID ? $ID : $NEW_ID),
		'SORT' => (array_key_exists('SORT', $_POST) ? $_POST['SORT'] : 100),
...
if ('' != $ID)
{
	...
	else
	{
		...
		{
			if (!array_key_exists($arOneLang['LID'], $arStatusLangs))
				continue;
		...
		if (!empty($arStatusGroupIDs))
		{
			...
			$rsPerms = CSaleStatus::GetList(
				array(),
				array('STATUS_ID' => $ID, 'GROUP_ID' => $arStatusGroupIDs),
				false,
				false,
				array('*')
			);
		...
		<td colspan="2" style="text-align: center;"><?
			...
			if (!$bReadOnly)
			{
				...
				echo GetMessage(
					...
					array(
						'#LINK#' => 'settings.php?lang='.LANGUAGE_ID.'&mid=sale&back_url_settings='.urlencode($APPLICATION->GetCurPageParam())
					)
				);
			}
		?></td>
