...
if (array_key_exists('dontsave', $_REQUEST) && 'Y' == $_REQUEST['dontsave'])
{
	$intLockUserID = 0;
	$strLockTime = '';
...
if ($REQUEST_METHOD == "POST" && $save_order_data == "Y" && empty($dontsave) AND $saleModulePermissions >= "U" AND check_bitrix_sessid())
{
	...
	if (!empty($arCupon) && is_array($arCupon))
		$recalcOrder = "Y";
	...
	if (isset($_POST["PRODUCT"]) && !empty($_POST["PRODUCT"]))
	{
		...
		{
			if (intval($val["PRODUCT_ID"]) > 0 && $val["MODULE"] == 'catalog' && $boolCatalog)
			{
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		if ($arOrder["TRACKING_NUMBER"] != $TRACKING_NUMBER)
							...
							if ('' == $errorMessageTmp && $DEDUCTED == "Y") //TODO? && $_POST["HAS_PRODUCTS_WITH_BARCODE_MULTI"] == "Y")
							{
								...
								foreach ($_POST["PRODUCT"] as $id => $arProduct)
								{
									if ($arProduct["BARCODE_MULTI"] == "Y" && is_array($arProduct["STORES"]) && !empty($arProduct["STORES"]))
									{
										foreach ($arProduct["STORES"] as $index => $arStore)
										{
											if (!is_array($arStore["BARCODE"]) || empty($arStore["BARCODE"]))
											{
											...
											if (!empty($arStore["BARCODE_FOUND"]) && !$bErrorFound)
											{
...
/*				$balance = 0;
			...
			if (!empty($arCatalogProduct) && CModule::IncludeModule('catalog'))
			{
...
foreach ($arBasketItem as $key => $val)
{
	if ($val["MODULE"] == "catalog")
	{
		if(!empty($arCatResult[$val["PRODUCT_ID"]]))
		{
			$arBasketItem[$key]["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$val["PRODUCT_ID"]
					."&type=".$arCatResult[$val["PRODUCT_ID"]]["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID="
...
<tr>
	...
	<table cellpadding="3" cellspacing="1" border="0" width="100%" class="internal" id="BASKET_TABLE">
		<tr id="heading_with_stores" class="heading" style="display:<? echo ($useStores && ($str_DEDUCTED == "Y" || $hasSavedBarcodes) ? 'table-row' : 'none');?>;">
			<td>&nbsp;</td>
		...
		<tr id="heading_without_stores" class="heading" style="display:<? echo ($useStores && ($str_DEDUCTED == "Y" || $hasSavedBarcodes) ? 'none' : 'table-row'); ?>;">
			<td>&nbsp;</td>
			<td><?echo GetMessage("SALE_F_PHOTO")?></td>
			<td><?echo GetMessage("SALE_F_NAME")?></td>
			<td><?echo GetMessage("SALE_F_QUANTITY")?></td>
	...
	?>
		<tr id="BASKET_TABLE_ROW_<?=$val["ID"]?>" onmouseover="fMouseOver(this);" onmouseout="fMouseOut(this);">
			...
			<td class="balance_count">
				<?
				$balance = "0";
				if ($val["MODULE"] == "catalog" && $boolCatalog)
				{
					$ar_res = CCatalogProduct::GetByID($val["PRODUCT_ID"]);
					$balance = FloatVal($ar_res["QUANTITY"]);
				}
			...
			<td id="td_store_block_<?=$val["ID"]?>" class="store" style="display:<? echo ($useStores && ($str_DEDUCTED == "Y" || $hasSavedBarcodes) ? '' : 'none'); ?>;">
			...
			<td class="order_price" style="white-space: nowrap;">
				<?
...
<tr>
	<td valign="top" align="left" colspan="2">
		...
		<div class="set_cupon">
			...
			<a href="javascript:void(0);" onclick="fRecallByCoupon();"><?=GetMessage("NEWO_CUPON_RECALC")?></a><sup style="color:#BE0000;">1)</sup>
		...
		<div style="float:right">
			<script type="text/javascript">
				...
				{
					var quantity = 1;
					var BUYER_ID = document.order_edit_info_form.user_id.value;
...
<script type="text/javascript">
	...
	function FillProductFields(index, arParams, iblockID)
	{
		...
		hiddenField += '<input type="hidden" name="BUSKET_' +  ID + '" id="BUSKET_' + ID + '" value="" />';
		...
		actonHtml = actonHtml + "[{'ICON':'view','TEXT':'<?=GetMessage("SOE_JS_EDIT")?>','ACTION':'ShowProductEdit("+ID+");','DEFAULT':true},{'ICON':'delete','TEXT':'<?=GetMessage("SOE_JS_DEL_WITHOUT_DISCOUNT")?>','ACTION':'DeleteProduct(this, "+ID+", false);fEnableSub();'}, {'ICON':'delete','TEXT':'<?=GetMessage("SOE_JS_DEL_WITH_DISCOUNT")?>','ACTION':'DeleteProduct(this, "+ID+", true);fEnableSub();'}]);\" class=\"adm-list-table-popup\"></div>";
	...
	function fRecallByCoupon()
	{
		var obCartFix = BX('CART_FIX');
		var obCoupon = BX('CUPON');
		if (!!obCartFix && 'Y' == obCartFix.value)
		{
			...
			<tr>
				<td valign="top" id="itog_tabs" class="load_product">
					...
					<div id="tabs">
						...
						if (empty($arRecomendetResult["ITEMS"]))
							$displayNone = "none";
...
if (isset($_GET["user_id"]) && isset($_GET["LID"]) && !$bVarsFromForm)
{
	...
	if ($boolCatalog
			...
			&& isset($_GET["product"])
			&& !empty($_GET["product"]) && is_array($_GET["product"]))
	{
		$bXmlId = COption::GetOptionString("sale", "show_order_product_xml_id", "N");
		$arProductId = array();
		...
		if (!empty($arGetProduct))
		{
			...
			while ($arItems = $res->GetNext())
			{
				...
				if (empty($arPrice["DISCOUNT"]) || !is_array($arPrice["DISCOUNT"]))
				{
				...
				$urlEdit = "/bitrix/admin/iblock_element_edit.php?ID=".$arItems["ID"]."&type=".$arItems["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arItems["IBLOCK_ID"]."&find_section_section=".intval($arItems["IBLOCK_SECTION_ID"]);

				$arParams = array(
					'id' => $arItems["ID"],
					'name' => $arItems["NAME"],
					'url' => $arItems["DETAIL_PAGE_URL"],
					'urlImg' => $ImgUrl,
					'urlEdit' => $urlEdit,
					'price' => $price,
					'priceFormated' => $price,
					'priceBase' => $arPrice["PRICE"]["PRICE"],
					'priceBaseFormat' => $arPrice["PRICE"]["PRICE"],
					'valutaFormat' => $priceValutaFormat,
					'priceDiscount' => $priceDiscount,
					'summaFormated' => $summaFormated,
					'priceTotalFormated' => $currentTotalPriceFormat,
