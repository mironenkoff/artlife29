...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		/* Check Values Begin */
		$arSelFields = array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "PRICE", "WEIGHT", "NAME", "CURRENCY", "CATALOG_XML_ID", "VAT_RATE", "NOTES", "DISCOUNT_PRICE", "PRODUCT_PROVIDER_CLASS");
		...
		foreach ($arResult["BASKET_ITEMS"] as &$arResultItem)
		{
			...
			if(intval($arProductPicture[$productId]["PREVIEW_PICTURE"]) > 0)
				$arResultItem["PREVIEW_PICTURE"] = $arProductPicture[$productId]["PREVIEW_PICTURE"];
			if(intval($arProductPicture[$productId]["DETAIL_PICTURE"]) > 0)
				$arResultItem["DETAIL_PICTURE"] = $arProductPicture[$productId]["DETAIL_PICTURE"];
		}
