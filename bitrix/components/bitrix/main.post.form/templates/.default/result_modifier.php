...
if ($arParams["TEXT"]["~SHOW"] != "Y")
{
	if ($addSpan) {
		array_unshift(
			...
			"<span ".
				"onclick=\"".
					"window['".$arParams["JS_OBJECT_NAME"]."'].showPanelEditor();".
					"if(window['".$arParams["JS_OBJECT_NAME"]."'].isPanelEditorShowed()){".
						"BX.userOptions.save('main.post.form', 'postEdit', 'showBBCode', 'Y');".
						"BX.addClass(this, 'feed-add-post-form-btn-active');".
					"}else{".
						"BX.userOptions.save('main.post.form', 'postEdit', 'showBBCode', 'N');".
						"BX.removeClass(this, 'feed-add-post-form-btn-active');".
					"}".
				"\" ".
		...
		$arParams["ADDITIONAL"][] =
			"{ text : '".GetMessage("MPF_EDITOR")."', onclick : function() {window['".$arParams["JS_OBJECT_NAME"]."'].showPanelEditor(); this.popupWindow.close(); if(window['".$arParams["JS_OBJECT_NAME"]."'].isPanelEditorShowed())	BX.userOptions.save('main.post.form', 'postEdit', 'showBBCode', 'Y'); else BX.userOptions.save('main.post.form', 'postEdit', 'showBBCode', 'N');}, className: 'blog-post-popup-menu', id: 'bx-html'}";
	}
}
